      ...
      <section className="px-4 sm:px-6 lg:px-8 py-12 sm:py-20">
        <div className="max-w-[1600px] mx-auto bg-[#fbf7e3] rounded-[2.5rem] px-6 py-16 md:py-24 relative overflow-hidden text-center text-black">
          ...
          <p className="relative z-10 text-black/70 text-xl max-w-2xl mx-auto mb-12 font-medium">
            We are here to help you with any questions or to schedule your next visit. Reach out to us today.
          </p>

          {/* Contact Info Grid */}
          <div className="relative z-10 grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {[
              { icon: Phone, title: "Call Us", text: "+91 77420 36693" },
              { icon: Mail, title: "Email Us", text: "drmalotdentalclinic@gmail.com" },
              { icon: MapPin, title: "Visit Us", text: "Malot Dental Clinic, Udaipur, India" },
            ].map((item, i) => (
              <div key={i} className="bg-white/60 backdrop-blur-md border border-white/50 p-6 rounded-3xl flex flex-col items-center gap-3 shadow-sm hover:scale-105 transition-transform duration-300">
                <div className="w-12 h-12 bg-black text-white rounded-full flex items-center justify-center">
                   <item.icon className="w-5 h-5" />
                </div>
                <h3 className="font-bold text-lg">{item.title}</h3>
                <p className="text-black/70 font-medium">{item.text}</p>
              </div>
            ))}
          </div>
        </div>
